...
1) Check Prerequisites :- 
   ...
   npm -v
   ```
   2. MetaMask installed in your browser (Chrome/Brave/Edge).
   3. Open a CMD window for each component you run (you’ll need multiple terminals).
...
```
   ...
   - terminal OPEN.
   - Important: Copy one of the Private Key strings (starts with 0x...). You’ll import that into MetaMask in a later step.

3) Deploy contracts to local Hardhat :-
- Open a new terminal
  ```
  cd C:\path\to\your\project\hardhat
  npx hardhat compile
...
```
  ...
  npm start
```
5) Prepare MetaMask (browser)
   
   https://metamask.io/download
   
   - Click Chrome Logo
   - Click " Add to Chrome"
   - Then click Shortcut ( After Downlode Complete)
   - Click a new Wallet (Login)
   - Click Three Line -- Networks -- Add Custom network
   ...
   - You Are Now on Hardhat Network
      - You should now see:
